...
#### Always use a domain layer with UseCases.
**Strongly recommend**

Every feature must include a domain layer containing dedicated UseCase classes.
Each UseCase encapsulates a single business operation and acts as the bridge between the Cubit and the Repository.
This enforces consistent structure across all features and makes business logic easy to locate, test, and extend as the app grows.
Even for simple pass-through operations, having a UseCase ensures structural consistency and makes future logic additions straightforward.
...
We recommend you use the `get_it` package to handle dependency injection.
