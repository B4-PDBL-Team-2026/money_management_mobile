...
class ProfileRemoteDataSource {
  ...
  Future<void> submitFinancialProfile(FinancialProfileModel payload) async {
    ...
    try {
      ...
      throw UnexpectedException(
        'Ada kendala pas simpan profil keuangan. Coba lagi ya.',
      );
    }
  }
  ...
  Future<void> updateBudgetLimits({
    ...
    if (AppEnv.useMockApi) {
      _log.info(
        'USE_MOCK_API enabled, simulating budget limits update success',
      );
      await Future.delayed(const Duration(seconds: 1));
      return;
    }
    ...
    try {
      await dio.patch(
        '/settings/dailyLimit',
        data: {'flooringLimit': safetyFlooring, 'ceilingLimit': safetyCeiling},
      );
    } on DioException catch (e) {
      ...
      throw UnexpectedException(
        'Ada kendala pas simpan perubahan budget harian. Coba lagi ya.',
      );
    }
  }
}
