...
///   "Beli soto ayam seharga Rp25.000 hari ini"   â† Rp prefix + dot separator
///   "Beli kopi di Kenangan seharga 35.000"
///   "Transfer 1.500.000 kemarin"
class TransactionInputParser {
  // â”€â”€ Normalization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  ///      "25.000" â†’ "25000",  "1.500.000" â†’ "1500000"
  ...
  static String _normalize(String text) {
    ...
    // Normalizing decimal comma â†’ dot  ("1,5jt" â†’ "1.5jt")
  ...
  static int? parseAmount(String text) {
    ...
    // Priority: juta â†’ ribu â†’ plain integer
    final patterns = <(RegExp, int)>[
      (RegExp(r'(\d+(?:\.\d+)?)\s*(?:jt|juta)\b'), 1000000),
      (RegExp(r'(\d+(?:\.\d+)?)\s*(?:rb|ribu|k)\b'), 1000),
      // Plain integer â‰¥ 3 digits, not immediately preceded by a letter
  ...
  static String parseName(String raw) {
    ...
    // Strip bare numbers â‰¥ 3 digits
    s = s.replaceAll(RegExp(r'\b\d{3,}\b'), '');
    // Strip filler keywords
    s = s.replaceAll(
      RegExp(
        r'\b(?:seharga|harga|senilai|sebesar|sejumlah)\b',
        caseSensitive: false,
      ),
      '',
    );
    s = s.replaceAll(
      RegExp(
        r'\b(?:hari ini|kemarin|lusa|tadi|besok|today|yesterday)\b',
        caseSensitive: false,
      ),
      '',
    );
  ...
  static ({String name, int id}) inferCategory(
    String text,
    ...
    if (type == TransactionType.income) {
      if (_any(lower, ['gaji', 'salary', 'upah']))
        return (name: 'Pendapatan', id: 11);
      if (_any(lower, ['bonus'])) return (name: 'Bonus', id: 12);
      if (_any(lower, ['hadiah', 'gift'])) return (name: 'Hadiah', id: 13);
      if (_any(lower, ['dividen', 'saham', 'investasi']))
        return (name: 'Investasi', id: 14);
      if (_any(lower, ['pinjaman', 'utang', 'hutang']))
        return (name: 'Pinjaman', id: 15);
